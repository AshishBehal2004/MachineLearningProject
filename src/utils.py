...
def evaluate_models(X_train, y_train, X_test, y_test, models ):
    ...
    try:
        for i in range(len(list(models))): #looping over every model in model dict.
            models = list(models.values())[i] # picks the actual model object at ith index

            models.fit(X_train, y_train) # Training the model

            y_train_pred = models.predict(X_train) # makes predictions on training data

            y_test_pred = models.predict(X_test) # make predictions on test data
